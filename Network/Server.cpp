...
void network::ServerInterface::SendMessageToValidClient(const Message& message, std::shared_ptr<ServerConnectionHandler> serverWithClientHandler)
{
...
void network::ServerInterface::SendMessageToOtherClients(const Message& message, std::shared_ptr<ServerConnectionHandler> serverWithClientHandler)
{
	for (auto& clientHandler : mEstablishedConnectionsWithClients)
		if (clientHandler != serverWithClientHandler && clientHandler) clientHandler->Send(message);
}
